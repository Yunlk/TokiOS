...
工具：`tools/gen_tk.py` — 两遍汇编器，rel8 跳转回填、绝对地址嵌入、字符串缓冲区，生成 shell.tk 机器码。
...
| 9 | `SYS_EXIT` | 退出进程 |
| 10 | `SYS_HEX` | hexdump 寄存器值 |
| 11 | `SYS_BACKSPACE` | 光标退格+擦除 |
| 12 | `SYS_LOGIN` | 登录认证（ebx=user, ecx=pass） |
| 13 | `SYS_EXEC` | 加载并替换当前进程（ebx=文件名） |
...
|------|-----|------|
| 0 | 内核 (tokisora) | 完全控制 |
| 3 | 所有用户 / .tk 进程 | 受 GDT DPL + 页表 U/S + IOPB 三重隔离 |

**提权**：`SYS_AUTH` 传入密码 → 内核 SHA-256 比对 `/etc/passwd`。  
**防暴力**：输错 3 次 → 30 秒锁死。  
**改密**：`passwd` 命令（ring0 shell）。  
**降权**：只能从 ring3 开始，提权是软状态（auth OK），不可升 ring。  
**存储**：密码哈希存 TFS 文件 `/etc/passwd`（小写 hex，64 字符）。  
**默认密码**：首次启动无文件时自动创建，默认 `tokios`。

### 密码原理
```
用户输密码 → SHA-256 → 64 字符 hex 哈希
↓
比对 /etc/passwd 中的 hex
↓
匹配 → auth OK
不匹配 → fail_cnt++ → 满 3 → 30s 锁
```
公钥不是必须的——密码是主要手段。密钥对方案留到网络栈阶段再做（Ed25519 验证器 ~200 行，远超当前规模）。
...
**2026-06-17**
多用户认证框架：`auth.c/h`，`/etc/passwd`（`user:SHA256`，最多 8 用户）。登录/注销/改密/新建用户命令。输错 3 次冻结 30 秒。默认账户 `tokisora`。SYS_MAX 边界检查。TokiFS v0.2：提取 `tfs_find()` 统一名字查找，删除紧凑回收。SYS_EXEC（#13）：ring3 进程加载并替换为新 .tk。修复 `tfs_get_size` 名字比较 break 位置 bug（input buffer 残留致误判 filenotfound）。VGA 滚屏（`scroll_up()`）和 UTF-8 乱码修复。

**2026-06-16**
shell.tk 完善：退格修复（专门加 `SYS_BACKSPACE` 在内核侧直接 VGA 擦除，不依赖 cursor_write 的 `\b` 处理）、Enter 换行修正、命令解析回显 bug（ebx 覆盖回路）修复。修复 TSS 结构体 `iopb/_res11` 字段顺序颠倒（x86 硬件在偏移 0x64 读 I/O Map Base，不是 0x66）。TSS IOPB 完工 — `iopb ≥ sizeof(tss_t)` 令 ring3 任何 `inb`/`outb` 触发 #GP，强制走系统调用。
...
| `syscall.c/h` | `int 0x80` 分发（SYS_COUT=1 ~ SYS_BACKSPACE=11） |
| `tfs.c/h` | TokiFS v0.2（16 槽位，删除紧凑回收，1MB RAM 盘） |
...
| 文件 | 内容 |
|------|------|
| `tools/gen_tk.py` | ring3 汇编器（两遍，rel8 回填，嵌入内核 shell_code） |
...
| TFS 线性分配，无法紧凑 | 紧凑 / 链表空闲块 | ✅ v0.2（删除→前移→next_free 缩减） |
| Ring 3 IOPB 未设置 | TSS IOPB ≥ sizeof(tss_t) | ✅ 已修复 |
