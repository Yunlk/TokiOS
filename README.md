...
```
boot.S        Multiboot/PVH 头, GDT/IDT flush, IRQ/系统调用汇编存根
kernel.c      清屏→初始化→进 TFS →预载 hello.tk→HLT
gdt.c/h       6 条目 GDT（ring0 代码/数据, ring3 代码/数据, TSS）
idt.c/h       中断向量, PIC 重映射, 0x80 trap gate (DPL=3)
isr.c/h       中断分派, regs 结构体, 键盘/光标 API 声明
keyboard.c    128 项查表→VGA 回显+缓冲+光标+ring3 键盘隔离
paging.c/h    恒等映射 0-4MB, page_map_user() 用户页
proc.c/h      进程加载（TFS→0x200000）, ring3 iret 跳转
shell.c/h     命令分发（函数指针表）, parse_args 参数解析
syscall.c/h   int 0x80 分发（SYS_COUT~SYS_EXIT）
tfs.c/h       ToyFS v0.1（16 槽位, 线性分配, 1MB RAM 盘）
linker.ld     链接脚本（Multiboot 头 + .bss ALIGN(4096)）
```
